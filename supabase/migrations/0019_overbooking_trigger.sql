-- ============================================================
-- Aggiorna trigger per permettere overbooking su slot ricorrenti.
...
begin
  ...
  -- Permetti overbooking solo per slot ricorrenti
  if v_slot.event_date is null then
    -- Slot ricorrente: permetti overbooking se non c'è spazio
    if v_count >= v_slot.max_capacity then
      new.is_overbooking := true;
    end if;
  else
    -- Slot evento: blocca se non c'è spazio
    if v_count >= v_slot.max_capacity then
